...
import type { TAppStackParamList, INavigationProps } from "./AppNavigator"
 ...
 */
export const navigationRef = createNavigationContainerRef<TAppStackParamList>()
...
export function getActiveRouteName(state: NavigationState | PartialState<NavigationState>): string {
  ...
  if (!route.state) {
    return route.name as keyof TAppStackParamList
  }

  // Recursive call to deal with nested routers
  return getActiveRouteName(route.state as NavigationState<TAppStackParamList>)
}
...
export function useNavigationPersistence(storage: Storage, persistenceKey: string) {
  const [initialNavigationState, setInitialNavigationState] =
    useState<INavigationProps["initialState"]>()
  const isMounted = useIsMounted()

  const initNavState = navigationRestoredDefaultState(Config.persistNavigation)
  const [isRestored, setIsRestored] = useState(initNavState)

  const routeNameRef = useRef<keyof TAppStackParamList | undefined>()
  ...
  const onNavigationStateChange = (state: NavigationState | undefined) => {
    ...
    if (state !== undefined) {
      ...
      routeNameRef.current = currentRouteName as keyof TAppStackParamList
  ...
  const restoreState = async () => {
    try {
      const state = (await storage.load(persistenceKey)) as INavigationProps["initialState"] | null
      if (state) {
